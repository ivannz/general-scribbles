...
Let $C$ be such a set. Note that by convexity of $C$ for any $x, h\in \Hcal$ we
have
\begin{equation*}
  % p(x)
  %   \leq p(x+h) + p(-h)
...
\begin{equation*}
  \bigl\lvert p(x + h) - p(x) \bigr\rvert
    ...
    \,.
\end{equation*}
Hence, if for any $x$ and any $\varepsilon > 0$ we can find an open $V$ such that
the right hand side supremum does not exceeed $\varepsilon$, then  we shall conclude
that $V \subseteq p^{-1}\bigl(B(p(x), \varepsilon))\bigr)$, and $p$ is continuous
at $x$.

To construct such $V$ let's fix some $\varepsilon > 0$. Observe that if $x \in \alpha
C$ for some $\alpha > 0$, then $p(x) \leq \alpha$ and thus $\alpha C \subseteq p^{-1}
\bigl([0, \alpha] \bigr)$. Since $0$ is in the interior of $C$, there is $\delta > 0$,
such that $B(0, \delta) \subseteq C$, which means that for any $\eta \in (0, 1)$
\begin{equation*}
  0 \in B(0, \varepsilon \eta \delta)
    ...
    \,,
\end{equation*}
since $[0, \varepsilon \eta] \subset [0, \varepsilon)$. Hence for any $\varepsilon > 0$
there is $\delta > 0$ with $B(0, \delta) \subseteq p^{-1}\bigl([0, \varepsilon) \bigr)$. 



On the other hand, if $x\in p^{-1}\bigl([0, \varepsilon)\bigr)$, then $x \in \lambda_x C
\subseteq \varepsilon C$ for some $\lambda_x \in [p(x), \varepsilon)$. Therefore
\begin{equation*}
  p^{-1}\bigl([0, \varepsilon)\bigr)
    ...
    \,.
\end{equation*}
Note that $\lambda_x C \subseteq \varepsilon C$ for $\lambda_x < \varepsilon$ follows from
$0\in C$ and convexity of $C$.


\begin{align*}
  p(x)
    &\leq p(x+h) + p(-h) < p(x+h) + \varepsilon
    \,, \\
  p(x + h)
    &\leq p(x) + p(h) < p(x) + \varepsilon
    \,,
\end{align*}
Thus $B(x, \delta) \in p^{-1}\bigl(B(p(x), \varepsilon)\bigr)$
